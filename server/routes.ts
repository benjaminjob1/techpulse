...
import {
  fetchLiveQuotes, fetchLiveGainers, fetchLiveLosers, fetchLiveMostActive,
  fetchLiveSentiment, fetchLivePriceHistory, getCacheTimestamp, invalidateCache,
} from "./finance-api";
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  app.get("/api/stocks", async (_req, res) => {
    try {
      ...
      if (liveData.length > 0) return res.json(liveData);
    } catch (err) { console.error("[routes] Live quotes failed, using fallback:", err); }
  ...
  app.get("/api/stocks/:symbol", async (req, res) => {
    ...
    } catch (err) { console.error("[routes] Live quote failed for", symbol); }
  ...
  app.get("/api/stocks/:symbol/history", async (req, res) => {
    ...
    try {
      ...
      if (liveHistory.length > 0) return res.json(liveHistory);
    } catch (err) { console.error("[routes] Live history failed for", symbol); }
  ...
  app.get("/api/market/gainers", async (_req, res) => {
    ...
    } catch (err) { console.error("[routes] Live gainers failed"); }
  ...
  app.get("/api/market/losers", async (_req, res) => {
    try {
      ...
      if (live.length > 0) return res.json(live);
    } catch (err) { console.error("[routes] Live losers failed"); }
  ...
  app.get("/api/market/active", async (_req, res) => {
    try {
      ...
      if (live.length > 0) return res.json(live);
    } catch (err) { console.error("[routes] Live most active failed"); }
  ...
  app.get("/api/market/sentiment", async (_req, res) => {
    try {
      ...
      if (live.sentiment !== "NEUTRAL" || live.marketStatus !== "unknown") return res.json(live);
    } catch (err) { console.error("[routes] Live sentiment failed"); }
  ...
  app.get("/api/data/timestamp", async (_req, res) => {
    ...
    res.json({ ...ts, fallbackData: storageTs.lastUpdated, source: storageTs.source });
  });
  ...
  app.post("/api/data/refresh", async (_req, res) => {
    ...
    try {
      ...
      res.json({ success: true, quotesCount: quotes.length, sentiment: sentiment.sentiment, refreshedAt: new Date().toISOString() });
    } catch (err) {
  ...
  app.get("/api/ai/analysis/:symbol", async (req, res) => {
    ...
    if (cached) {
      ...
      if (age < 30 * 60 * 1000) return res.json(cached);
    }
    let stock;
    try {
      const liveData = await fetchLiveQuotes();
      stock = liveData.find(s => s.symbol === symbol);
    } catch { stock = storage.getStock(symbol); }
  ...
  app.get("/api/forecast/:symbol", async (req, res) => {
    ...
    if (cached) {
      const age = Date.now() - new Date(cached.generatedAt).getTime();
      if (age < 60 * 60 * 1000) return res.json(cached);
    }
    let stock;
    try {
      const liveData = await fetchLiveQuotes();
    ...
    const target = storage.getAnalystTarget(symbol);
    let history;
    try { history = await fetchLivePriceHistory(symbol); } catch { history = storage.getPriceHistory(symbol); }
    if (!history || history.length === 0) history = storage.getPriceHistory(symbol);
    try {
  ...
  app.post("/api/ai/analyze-all", async (_req, res) => {
    ...
    let stocks;
    try { stocks = await fetchLiveQuotes(); } catch { stocks = storage.getStocks(); }
    ...
    for (const symbol of topSymbols) {
      try {
        ...
        if (cached) {
          const age = Date.now() - new Date(cached.generatedAt).getTime();
          if (age < 30 * 60 * 1000) { results.push({ symbol, status: "cached" }); continue; }
        }
        const stock = stocks.find(s => s.symbol === symbol);
        if (!stock) { results.push({ symbol, status: "not_found" }); continue; }
        const target = storage.getAnalystTarget(symbol);
        const analysis = await generateAIAnalysis(stock, target);
        storage.setAIAnalysis(symbol, analysis);
        results.push({ symbol, status: "generated" });
      } catch (err) { results.push({ symbol, status: "failed" }); }
    }
