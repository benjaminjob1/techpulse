...
function parseNumber(val: string): number {
  ...
  // Remove commas from numbers like "4,380,976,286,411.00"
...
export async function fetchLiveQuotes(): Promise<StockQuote[]> {
  ...
  try {
    ...
    const quotes: StockQuote[] = rows
      ...
      .map(r => ({
        ...
        changesPercentage: parseNumber(r.changesPercentage),
        marketCap: parseNumber(r.marketCap),
        pe: parseNumber(r.pe),
        eps: parseNumber(r.eps),
        volume: parseNumber(r.volume),
        avgVolume: parseNumber(r.avgVolume),
        dayLow: parseNumber(r.dayLow),
        dayHigh: parseNumber(r.dayHigh),
        yearLow: parseNumber(r.yearLow),
        yearHigh: parseNumber(r.yearHigh),
        previousClose: parseNumber(r.previousClose),
  ...
  return []; // fallback empty — storage.ts has hardcoded backup
}
...
export async function fetchLiveGainers(): Promise<MarketMover[]> {
  ...
  try {
    const result = callFinanceTool("finance_market_gainers", {
      query: "Top gaining stocks today",
      action: "Fetching top market gainers",
      limit: 8,
    });

    const rows = parseAllTables(result.content);
    const movers: MarketMover[] = rows
      .filter(r => r.symbol)
      .slice(0, 8)
      .map(r => ({
        symbol: r.symbol,
    ...
    if (movers.length > 0) {
      setCache("gainers", movers);
      return movers;
    }
  } catch (err) {
    console.error("[finance-api] Failed to fetch gainers:", err);
  }
...
export async function fetchLiveLosers(): Promise<MarketMover[]> {
  ...
  try {
    const result = callFinanceTool("finance_market_losers", {
      query: "Top losing stocks today",
      action: "Fetching top market losers",
      limit: 8,
    });
    ...
    if (movers.length > 0) {
      setCache("losers", movers);
      return movers;
    }
  } catch (err) {
    console.error("[finance-api] Failed to fetch losers:", err);
  }
...
export async function fetchLiveMostActive(): Promise<MarketMover[]> {
  ...
  try {
    const result = callFinanceTool("finance_market_most_active", {
      query: "Most actively traded stocks",
      action: "Fetching most active stocks",
      limit: 8,
    });
    ...
    if (movers.length > 0) {
      setCache("most_active", movers);
      return movers;
    }
  } catch (err) {
    console.error("[finance-api] Failed to fetch most active:", err);
  }
...
export async function fetchLiveSentiment(): Promise<MarketSentiment> {
  ...
  try {
    const result = callFinanceTool("finance_market_sentiment", {
      query: "US stock market sentiment",
    ...
    console.error("[finance-api] Failed to fetch sentiment:", err);
  }
...
export async function fetchLivePriceHistory(symbol: string): Promise<PricePoint[]> {
  ...
  try {
    ...
    console.error(`[finance-api] Failed to fetch history for ${symbol}:`, err);
  }
