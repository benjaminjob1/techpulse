...
export default function StockDetail() {
  ...
  const { data: stock, isLoading } = useQuery<StockQuote>({
  ...
  const { data: allTargets } = useQuery<AnalystTarget[]>({
  ...
  const { data: forecast, isLoading: forecastLoading } = useQuery<PriceForecast>({
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg font-semibold mb-2">Stock not found</p>
      ...
      <header className="sticky top-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors" data-testid="back-link">
            ...
            <div className="text-right">
              ...
              <div className={`flex items-center justify-end gap-1 text-sm font-medium ${
                ...
                <span className="tabular-nums">
                  {isPositive ? "+" : ""}{stock.change.toFixed(2)} ({isPositive ? "+" : ""}{stock.changesPercentage.toFixed(2)}%)
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={history}>
                        ...
                        <XAxis
                          ...
                          interval={Math.floor((history?.length || 60) / 8)}
                        />
                        <YAxis
                          tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }}
                          axisLine={false}
                ...
                {forecastLoading ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border ${
                        forecast.outlook === "BULLISH" ? "bg-emerald-500/15 border-emerald-500/30 text-emerald-400" :
                        forecast.outlook === "BEARISH" ? "bg-red-500/15 border-red-500/30 text-red-400" :
                        "bg-amber-500/15 border-amber-500/30 text-amber-400"
                      }`}>
                        {forecast.outlook === "BULLISH" ? <TrendingUp className="w-4 h-4" /> :
                         forecast.outlook === "BEARISH" ? <TrendingDown className="w-4 h-4" /> :
                         <Minus className="w-4 h-4" />}
                        <span className="text-sm font-bold">{forecast.outlook}</span>
                        <span className="text-xs opacity-70">Outlook</span>
                      </div>
                          ...
                          <div key={f.period} className="p-3 rounded-lg bg-background/60 border border-border/40">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-bold bg-violet-500/15 text-violet-400 px-2 py-0.5 rounded">{f.label}</span>
                                <Badge variant="secondary" className="text-[10px] px-1.5 py-0">{f.confidence}</Badge>
                              </div>
                              <div className="text-right">
                                <span className="text-sm font-bold tabular-nums">${f.mid.toFixed(2)}</span>
                                <span className={`ml-1.5 text-xs font-medium tabular-nums ${isUp ? "text-emerald-500" : "text-red-500"}`}>
                                  {isUp ? "+" : ""}{f.changePercent.toFixed(1)}%
                                </span>
                              </div>
                            </div>
                            {/* Range bar */}
                            <div className="relative h-2 bg-muted rounded-full overflow-visible">
                              <div
                                className="absolute h-full bg-gradient-to-r from-violet-500/30 to-violet-500/50 rounded-full"
                                style={{ left: "0%", width: "100%" }}
                              />
                              {/* Current price marker */}
                              <div
                                className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-foreground/70 rounded-full border border-background"
                                style={{ left: `${Math.min(Math.max(currentInRange, 2), 98)}%` }}
                                title={`Current: $${forecast.currentPrice.toFixed(2)}`}
                              />
                              {/* Mid target marker */}
                              <div
                                className="absolute top-1/2 -translate-y-1/2 w-1 h-4 bg-violet-500 rounded-full"
                                style={{ left: `${Math.min(Math.max(midInRange, 2), 98)}%` }}
                                title={`Target: $${f.mid.toFixed(2)}`}
                              />
                            </div>
                            <div className="flex justify-between mt-1">
                              <span className="text-[10px] text-muted-foreground tabular-nums">${f.low.toFixed(0)} Low</span>
                              <span className="text-[10px] text-muted-foreground tabular-nums">High ${f.high.toFixed(0)}</span>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/30">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">{forecast.methodology}</p>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-xs font-medium text-amber-500">Risk Factors</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {forecast.riskFactors.map((r, i) => (
                          <span key={i} className="text-xs px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-400/80 border border-amber-500/20">
                            {r}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Price forecast will be generated automatically.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card className="border border-primary/20 bg-primary/[0.02]">
              ...
              <CardContent>
                {aiLoading ? (
                ...
                ) : aiAnalysis && aiAnalysis.symbol ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <SignalBadge signal={aiAnalysis.signal} confidence={aiAnalysis.confidence} size="lg" />
                      <span className="text-[10px] text-muted-foreground">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="text-xs font-semibold text-emerald-500">Bull Case</span>
                        </div>
                        <p className="text-xs text-muted-foreground leading-relaxed">{aiAnalysis.bullCase}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-red-500/5 border border-red-500/20">
                        <div className="flex items-center gap-1.5 mb-1">
                          <TrendingDown className="w-3.5 h-3.5 text-red-500" />
                          <span className="text-xs font-semibold text-red-500">Bear Case</span>
                        </div>
                        <p className="text-xs text-muted-foreground leading-relaxed">{aiAnalysis.bearCase}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-xs text-muted-foreground mb-2 font-medium">Key Factors</p>
                      <div className="flex flex-wrap gap-1.5">
                        {aiAnalysis.keyFactors.map((f, i) => (
                          <span key={i} className="text-xs px-2 py-0.5 rounded-md bg-muted/80 text-foreground/80 border border-border/50">
                            {f}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-xs text-muted-foreground mb-3">
                      AI analysis will be generated automatically when data is available.
                    </p>
                    <p className="text-[10px] text-muted-foreground">
                      Click "Generate" on the dashboard to create analyses for top stocks.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          <div className="space-y-6">
            ...
            <Card className="border border-border/50">
              ...
              <CardContent>
                ...
                {stock.dividendYieldTTM > 0 && (
                  <StatRow label="Div Yield" value={`${(stock.dividendYieldTTM * 100).toFixed(2)}%`} />
                )}
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="relative h-2 bg-muted rounded-full mb-2">
                  <div
                    ...
                    style={{ left: `${Math.min(Math.max(rangePos, 3), 97)}%` }}
                  />
                </div>
                <div className="flex justify-between">
                  <div>
                    <p className="text-[10px] text-muted-foreground">Low</p>
                    <p className="text-xs font-medium tabular-nums">${stock.yearLow.toFixed(2)}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] text-muted-foreground">Current</p>
                    <p className="text-xs font-semibold tabular-nums text-primary">${stock.price.toFixed(2)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-muted-foreground">High</p>
                    <p className="text-xs font-medium tabular-nums">${stock.yearHigh.toFixed(2)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="pt-2">
                    <div className="flex justify-between mb-1">
                      <span className="text-xs text-muted-foreground">Vol vs Avg</span>
                      <span className={`text-xs font-medium tabular-nums ${
                        volumeRatio > 1.2 ? "text-amber-500" : volumeRatio > 1 ? "text-emerald-500" : "text-muted-foreground"
                      }`}>
                        {(volumeRatio * 100).toFixed(0)}%
                      </span>
                    </div>
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          volumeRatio > 1.2 ? "bg-amber-500" : volumeRatio > 1 ? "bg-emerald-500" : "bg-muted-foreground/30"
                        }`}
              ...
              <Card className="border border-border/50">
                ...
                <CardContent>
                  <div className="text-center mb-3">
                    <Badge variant="secondary" className="text-xs mb-1">{target.consensusRating}</Badge>
                    <p className="text-lg font-bold tabular-nums">${target.targetConsensus.toFixed(0)}</p>
                  ...
                  <div className="flex justify-between mt-1">
                    <span className="text-[10px] text-emerald-500">{target.bullishPct}% Bull</span>
                    <span className="text-[10px] text-amber-500">{target.neutralPct}% Neutral</span>
                    <span className="text-[10px] text-red-500">{target.bearishPct}% Bear</span>
                  </div>
                </CardContent>
              </Card>
            )}
